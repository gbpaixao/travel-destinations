...
export class DestinationsFakeAPI implements DestinationsAPI {
  constructor(private calculateDistance: CalculateDistance) { }

  delay<T>(result: T, milliseconds = 2000): Promise<T> {
  ...
  private getFiveClosestDestinations(destination: Destination) {
    const validDestinations = destinations.filter(dest => dest.id !== destination.id)
    const distances = validDestinations.map((dest) => ({
      destination: dest,
      distance: this.calculateDistance.execute({
        lat1: destination.latitude, 
        long1: destination.longitude, 
        lat2: dest.latitude, 
        long2: dest.longitude
      })
    }));
    distances.sort((a, b) => a.distance - b.distance);
    return distances.slice(0, 5).map(entry => entry.destination);
  }

  async searchDestinationByName(name: string): Promise<Destination[]> {
    console.log('API <searchDestinationByName> [name]:', name)
    if (name === "") return []
    if (name === "fail") throw new Error("Something bad happened")
    const filteredDestinations = destinations.filter((destination) => destination.name.toLowerCase().includes(name.toLocaleLowerCase()));
    return this.delay<Destination[]>(filteredDestinations)
  }

  async findNearbyDestinations(id: number): Promise<Destination[]> {
    console.log('API <findNearbyDestinations> [id]:', id)
    const destination = destinations.find(destination => destination.id === id)
    if (!destination) throw new Error('Destination not found')
    const fiveClosestDestinations = this.getFiveClosestDestinations(destination)
    return this.delay<Destination[]>(fiveClosestDestinations)
  }
}